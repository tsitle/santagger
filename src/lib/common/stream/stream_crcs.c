/******************************************************************************/
/* stream_crcs.c                [Sandy Tagger | Lib]                     tw=4 */
/*                                                                            */
/* (C) 2005-2024 by Thomas Sandkuehler (technisandk@gmail.com)                */
/*                                                                            */
/* Functions for CRC computation                                              */
/******************************************************************************/
/*                                                                            */
...
#undef SRC_STREAM_CRCS_C

/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/

/** CRC-8, poly = x^8 + x^2 + x^1 + x^0, init = 0 */
const Tst_byte ST_STREAM_CRC8_TABLE[256] = {
...
const Tst_uint16 ST_STREAM_CRC16_TABLE[256] = {
...
const Tst_uint32 ST_STREAM_CRC32_TABLE[256] = {
...
st_stream_pauseOrContinueCRC8(Tst_stream_crcs *pCRCs)
{
	...
	if (pCRCs->compCRC8) {
		pCRCs->pausCRC8 = ST_B_TRUE;
		pCRCs->compCRC8 = ST_B_FALSE;
	} else if (pCRCs->pausCRC8) {
		...
		const Tst_buf *pBuf, const Tst_uint32 sz)
{
	...
	ST_ASSERTN_VOID(pCRCs == NULL || pBuf == NULL)

	if (pCRCs->compCRC8) {
		pB = pBuf;
		pE = pB + sz;
		while (pB < pE)
			ST_STREAM_UPDATE_CRC8(*pCRCs, *pB++)
	}
}
...
st_stream_pauseOrContinueCRC16(Tst_stream_crcs *pCRCs)
{
	...
	if (pCRCs->compCRC16) {
		pCRCs->pausCRC16 = ST_B_TRUE;
		pCRCs->compCRC16 = ST_B_FALSE;
	} else if (pCRCs->pausCRC16) {
		...
		const Tst_buf *pBuf, const Tst_uint32 sz)
{
	...
	if (pCRCs->compCRC16) {
		...
		pE = pB + sz;
		while (pB < pE)
			ST_STREAM_UPDATE_CRC16(*pCRCs, *pB++)
	}
}
 ...
 *   polynomial=0x04c11db7, initialized with 0
 */
void
...
st_stream_pauseOrContinueCRC32(Tst_stream_crcs *pCRCs)
{
	...
	if (pCRCs->compCRC32) {
		pCRCs->pausCRC32 = ST_B_TRUE;
		pCRCs->compCRC32 = ST_B_FALSE;
	} else if (pCRCs->pausCRC32) {
		...
		const Tst_buf *pBuf, const Tst_uint32 sz)
{
	...
	if (pCRCs->compCRC32) {
		...
		pE = pB + sz;
		while (pB < pE)
			ST_STREAM_UPDATE_CRC32(*pCRCs, *pB++)
	}
}
